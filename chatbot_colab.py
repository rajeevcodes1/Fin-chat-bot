...
Original file is located at
    https://colab.research.google.com/drive/1TiHrVXRyJVi88UJiQArqj51Xym90iyhl
"""

"""
SEC EDGAR + yfinance — Enriched Financial Dataset
===================================================
Raw Metrics (from EDGAR):
  Total Revenue, Gross Profit, Operating Income, Net Income,
  Total Assets, Current Assets, Total Liabilities, Current Liabilities,
  Total Equity, R&D Expenses, EPS (Diluted), Shares Outstanding,
...
Market Data (from yfinance — historical per fiscal year):
  Stock Price, 52-Week High, 52-Week Low,
  Market Cap ($M), P/E Ratio, Price-to-Sales, Beta (current only)
"""
...
# ── EDGAR XBRL Tags ────────────────────────────────────────────────────────────
# Each metric lists fallback tags in priority order
EDGAR_METRICS = {
    # ── Income Statement ──
    "Total Revenue": [
    ...
    "Shares Outstanding": [
        "WeightedAverageNumberOfSharesOutstandingBasic",  # duration, fp=FY — always matches fiscal year-end date
        "CommonStockSharesOutstanding",                    # instant, no fp — fallback only
    ],
    # ── Cash Flow ──
    "Cash Flow from Operations": [
...
# Metrics reported in USD (convert to $B in output)
USD_METRICS = {
    "Total Revenue", "Gross Profit", "Operating Income", "Net Income",
...
def fetch_edgar_data(cik: str) -> tuple[str, dict]:
...
def fetch_historical_market_data(ticker: str, fiscal_year_ends: list) -> dict:
    ...
    Mirrors the standalone test approach — no index arithmetic, just iloc on filtered windows.
    """
    ...
    try:
        ...
        # Strip timezone — same as standalone test
        ...
        for date_str in fiscal_year_ends:
            ...
            # ── Same window filter as standalone test ──
            ...
            # (avoids index arithmetic — just slice and take last row)
...
def calculate_market_ratios(row: dict) -> dict:
    ...
    Shares Outstanding is in raw units; Revenue is in $M.
    """
...
def safe_divide(numerator, denominator):
    """Divide two values safely, returning None if invalid."""
    if numerator is None or denominator is None or denominator == 0:
        return None
    return round(numerator / denominator * 100, 2)  # returns as percentage


def safe_ratio(numerator, denominator, decimals=2):
    """Return a plain ratio (not percentage)."""
    if numerator is None or denominator is None or denominator == 0:
        return None
...
def build_dataset() -> pd.DataFrame:
    ...
    for company in COMPANIES:
        ...
        # ── yfinance: historical market data for all fiscal year-end dates ──
        ...
        for date in recent_dates:
            ...
            # Historical market data for this specific year
        ...
        # ── Market-based ratios (P/E, Market Cap, Price-to-Sales) per year ──
...
# OUTPUT_PATH = "/content/drive/MyDrive/Bootcamp - project/financials_enriched.csv"

# Option B: Direct browser download (Colab)
# from google.colab import files
# df.to_csv("financials_enriched.csv", index=False)
# files.download("financials_enriched.csv")

# Option C: Local path (uncomment if running locally)
# OUTPUT_PATH = "/Users/mengyao/Documents/Bootcamp - project/financials_enriched.csv"

# ── Active export (change to your preferred option above) ─────────────────────
import os
OUTPUT_PATH = "/content/drive/MyDrive/Bootcamp - project/financials_enriched.csv"
...
=========================
Plain-text analysis functions built on top of the enriched DataFrame.
Designed to be called by a chatbot or notebook.

Usage:
    df = build_dataset()   # from edgar_enriched.py
    print(company_snapshot(df, "Microsoft"))
    print(compare_companies(df, metric="Net Profit Margin (%)"))
    print(trend_analysis(df, "Apple"))
"""
...
def arrow(current, previous) -> str:
    """Return trend arrow based on direction of change."""
...
def company_snapshot(df: pd.DataFrame, company_name: str) -> str:
    """
    Full snapshot of a company's most recent fiscal year.
    Covers profitability, balance sheet, cash flow, and market valuation.
    """
...
def profitability_summary(df: pd.DataFrame, company_name: str) -> str:
    """How profitable has this company been over the past 5 years?"""
    rows = get_company(df, company_name)
...
def balance_sheet_health(df: pd.DataFrame, company_name: str) -> str:
    """Is this company's balance sheet strong or stretched?"""
    rows = get_company(df, company_name)
    latest = rows.iloc[0]
    name   = latest["Company"]
    year   = latest["Year"]
...
def growth_analysis(df: pd.DataFrame, company_name: str) -> str:
    """How fast has this company grown revenue and earnings?"""
    rows = get_company(df, company_name)
    name = rows.iloc[0]["Company"]
...
def compare_companies(df: pd.DataFrame, metric: str, year: int = None) -> str:
    """
    Rank all companies by a given metric for the most recent (or specified) year.
    Example: compare_companies(df, "Net Profit Margin (%)")
    """
    if year is None:
        year = df["Year"].max()
...
def trend_analysis(df: pd.DataFrame, company_name: str) -> str:
    """Detect which metrics are at 5-year highs/lows and which are trending up/down."""
    rows = get_company(df, company_name)
    name = rows.iloc[0]["Company"]
...
def full_report(df: pd.DataFrame, company_name: str) -> str:
    """Generate a complete analysis report for a company."""
    sections = [
...
# ── Example Usage ──────────────────────────────────────────────────────────────
# Uncomment and run after building df with edgar_enriched.py:
#
print(company_snapshot(df, "Microsoft"))
print(profitability_summary(df, "Apple"))
print(balance_sheet_health(df, "Tesla"))
print(growth_analysis(df, "Microsoft"))
print(compare_companies(df, "Net Profit Margin (%)"))
print(compare_companies(df, "Market Cap ($M)"))
print(trend_analysis(df, "Apple"))
print(full_report(df, "Tesla"))

!pip install anthropic

"""
Financial Analysis Chatbot — Colab Version
===========================================
Connects Claude to your enriched financial DataFrame via Anthropic tool use.
The LLM decides which analysis function to run based on your question.
...
Usage:
    df = build_dataset()          # from edgar_enriched.py
    run_chatbot(df)               # starts the chat loop
"""
...
# ── Anthropic Client ───────────────────────────────────────────────────────────
# Store your key in Colab Secrets (left sidebar → 🔑) as ANTHROPIC_API_KEY
...
data for Microsoft, Apple, and Tesla covering the past 5 fiscal years.
...
def ask(question: str, history: list, df) -> str:
    ...
    - tool args are already a dict (no json.loads needed)
    - tool results go back as user messages with type "tool_result"
    - stop_reason == "tool_use" signals a tool call (not msg.tool_calls)
    """
    ...
    while response.stop_reason == "tool_use":
        ...
        for block in response.content:
            if block.type == "tool_use":
                ...
                result = dispatch_tool(block.name, block.input, df)  # .input is already a dict
                tool_results.append({
                    "type"       : "tool_result",
        ...
        # ── Next call: Claude composes final answer using tool results ──
        response = client.messages.create(
            model=MODEL,
            max_tokens=4096,
            system=SYSTEM_PROMPT,
...
def run_chatbot(df):
    """
    Start an interactive chat loop in the Colab notebook.
    ...
    while True:
        ...
        # Keep conversation history for context (text only — no tool call blocks)
